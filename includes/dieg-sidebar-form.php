...
<div> 
URL страницы записи : <p class="howto"></p> 
...
<div id="tab-1" class="tab-content current">
	<label style="width: 85px;display: inline-block;"><?php 

//echo $metas;

/*unset($metas['fb-tolstelka']);
unset($metas['amushtaev']);
unset($metas['google']);
print_r($metas);*/
...
//$a = array_fill_keys($metas, 'fb_tolstelka');
//print_r($a);
//print_r($metas['fb_tolstelka']);

	/*if ( array_key_exists("fb_tolstelka", $metas)) {
	    echo "Нашел fb_tolstelka";

	    foreach ($meta_key_fb as $key => $value) {
		    echo "{$key} => {$value} ";
		    print_r($meta_key_fb);
		}
	} else {
		echo "no Нашел fb_tolstelka";
	}*/
